...
//  Content:    The axis aligned box class.
//  $Id:        AxisAlignedBox.h 101 2010-03-01 20:43:38Z agray $
//
//////////////////////////////////////////////////////////////////////////////
...
#ifndef __ZSPACE_AXIS_ALIGNED_BOX_H__
#define __ZSPACE_AXIS_ALIGNED_BOX_H__
...
*/
	...
	/** A 3D box aligned with the <i>x</i>, <i>y</i>, and <i>z</i> axes.
	@remarks
	This class represents a simple box that is aligned with the
	axes. Internally it only stores 2 points as the extremeties of
	the box. One of the points is the minima of all 3 axes. The other
	point is the maxima of all 3 axes. This class is typically used
	for an axis-aligned bounding box (AABB) for collision and
	visibility determination.
	*/
	class ZSPACE_COMMON_API AxisAlignedBox
	{
	public:
		enum Extent
		{
	...
	public:
		...
		*/
		...
		{
            ZS_ASSERT(0x000000f8, min.x <= max.x && min.y <= max.y && min.z <= max.z,
		...
		inline void setExtents(
			...
			float Mx, float My, float Mz )
		{
            ZS_ASSERT(0x000000f9, mx <= Mx && my <= My && mz <= Mz,
                "The minimum corner of the box must be less than or equal to maximum corner" );
		...
		@remarks
		The order of these corners is important. Because Ogre uses right-handed coordinates,
		the minimum Z is at the back of the box. See the following diagram for an
		explanation of the order of the points:
		<pre>
		   1-----2
		  /|    /|
		 / |   / |
		...
		*/
		...
		{
			ZS_ASSERT(0x000000fa, mExtent == EXTENT_FINITE, "Can't get corners of a null or infinite AxisAlignedBox");

			// The order of these items is, using right-handed co-ordinates:
			// Minimum Z face, starting with Min(all), then counterclockwise
			//   around face (looking onto the face)
			// Maximum Z face, starting with Max(all), then counterclockwise
			//   around face (looking onto the face)
		...
		*/
		...
		ZSPACE_COMMON_API friend std::ostream& operator<<( std::ostream& o, const AxisAlignedBox aab )
		{
			switch (aab.mExtent)
			{
			case EXTENT_NULL:
			...
			default: // shut up compiler
				ZS_ASSERT(0x000000fb, false, "Never reached");
				return o;
			}
		}
		...
		*/
		inline void merge( const Vector3& point )
		{
			...
			ZS_ASSERT(0x000000fc, false, "Never reached");
		}

		/** Transforms the box according to the matrix supplied.
		@remarks
		By calling this method you get the axis-aligned box that
		surrounds the transformed version of this box. Each
		corner of the box gets transformed by the matrix. The
		extents are mapped then back onto the axes to produce another
		AABB. This is useful when you have a local AABB for an object that
		...
		*/
		inline void transform( const Matrix4& matrix )
		{
			...
			// that gives the resulting point and merge the resulting point.
		...
		By calling this method you get the axis-aligned box that
		surrounds the transformed version of this box. Each
		corner of the box gets transformed by the matrix then the
		...
		*/
		...
		{
			ZS_ASSERT(0x000000fd, m.isAffine(), "Non-affine transform given to transformAffine.");
		...
		*/
		...
		float volume(void) const
		{
			...
			default: // shut up compiler
				ZS_ASSERT(0x000000fe, false, "Never reached");
				return 0.0f;
			}
		}
		...
		bool intersects(const Vector3& v) const
		{
			...
			default:
				ZS_ASSERT(0x000000ff, false, "Never reached");
				return false;
			}
		}
		...
		{
			ZS_ASSERT(0x00000100, mExtent == EXTENT_FINITE, "Can't get center of a null or infinite AxisAlignedBox");
		...
		Vector3 getSize(void) const
		{
			...
			default:
				ZS_ASSERT(0x00000101, false, "Never reached");
				return Vector3::ZERO();
			}
		}
		...
		Vector3 getHalfSize(void) const
		{
			...
			default:
				ZS_ASSERT(0x00000102, false, "Never reached");
				return Vector3::ZERO();
			}
		}
...
#endif // __ZSPACE_AXIS_ALIGNED_BOX_H__
